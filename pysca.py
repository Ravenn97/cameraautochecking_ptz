...
# along with this program.  If not, see <http://www.Fgnu.org/licenses/>.
        ...
        with self.__cond:
            if self.__waiting > 0:
                if self.__status == Socket.READY:
                    print "WARNING: Clearing socket {} found {} threads waiting but status was READY".format(
                        self.__number, self.__waiting)
...
def set_brightness(device, action, blocking=False):
    ...
    try:
        ...
        raise ValueError("The string '{0}', '{1}' or a one-byte-long integer must be passed " \
                         "when adjusting the Brightnefacess".format(BRIGHT_ACTION_UP, BRIGHT_ACTION_DOWN))
